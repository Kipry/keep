// Renders keep-promo.html to an MP4, one deterministic frame at a time.
//
//   node render.mjs                 → keep-promo-20s.mp4
...
const FPS = 30, SECONDS = 20, W = 1080, H = 1920;
const args = process.argv.slice(2);
const stillAt = args[0] === '--still' ? parseFloat(args[1]) : null;

const browser = await chromium.launch(process.env.CHROMIUM ? { executablePath: process.env.CHROMIUM } : {});
const page = await browser.newPage({ viewport: { width: W, height: H }, deviceScaleFactor: 1 });
await page.goto(pathToFileURL(path.join(here, 'keep-promo.html')).href);
...
const out = path.join(here, 'keep-promo-20s.mp4');
